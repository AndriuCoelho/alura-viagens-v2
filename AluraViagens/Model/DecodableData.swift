...
func load(_ filename: String) -> [ViagemViewModel]? {
    ...
    do {
        ...
        for sessao in listaDeViagens.keys {
            ...
            case .internacionais:
                if let internacionais = tiposDeViagens?.internacionais {
                    let internacionalViewModel = ViagemInternacionalViewModel(internacionais)
                    listaViagemViewModel.append(internacionalViewModel)
                }
